...
@then("I'm not logged in")  # noqa: F811
@given("I'm not logged in")  # noqa: F811
def step_impl(context):
    WebDriverWait(context.browser, 10).until(
...
@when(
    'I enter valid credentials "{username}" and "{password}" and push the login button'
)  # noqa: E501,F811
...
@when(
    'I enter invalid credentials "{username}" and "{password}" and push the login button'
)  # noqa: E501,F811
def step_impl(context, username, password):
    login_button = context.browser.find_element(By.ID, "login_button")
    username_field = context.browser.find_element(By.ID, "id_username")
...
@when('I create "{username}" user with "{password}" password')  # noqa: F811
def step_impl(context, username, password):
    username_field = context.browser.find_element(By.ID, "id_username")
    password_field = context.browser.find_element(By.ID, "id_password1")
    password2_field = context.browser.find_element(By.ID, "id_password2")
...
@when('I add a new "{bio}" and click the update button')  # noqa: F811
...
@when("I click the edit profile button")  # noqa: F811
def step_impl(context):
...
@when("I click the log out button")  # noqa: F811
def step_impl(context):
