...
@when('I write a note to "{friend}" with "{content}"')  # noqa: F811
...
@when('I write a reply to "{friend}" with "{content}"')  # noqa: F811
def step_impl(context, friend, content):
...
@when("I click 'start a conversation'")  # noqa: F811
...
@when('I click on the note from "{friend}"')  # noqa: F811
...
@when('I click on the note from "{friend}" on the conversation page')  # noqa: E501,F811
def step_impl(context, friend):
    note_link = context.browser.find_element(By.PARTIAL_LINK_TEXT, friend)
    paragraph = note_link.find_element_by_css_selector("p")
    paragraph.click()
...
@when("I click the back button")  # noqa: F811
def step_impl(context):
...
def step_impl(context):
    WebDriverWait(context.browser, 10).until(
        EC.presence_of_element_located((By.ID, "send_button"))
    )
...
def step_impl(context, friend):
    WebDriverWait(context.browser, 10).until(
        EC.presence_of_element_located((By.PARTIAL_LINK_TEXT, friend))
    )
