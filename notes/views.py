...
from .forms import ReplyForm, WriteNoteForm
...
def dash(request):
    ...
    for friend_pk in friends_pks:
        if friend_pk == request.user.pk:
            conversation = all_notes.filter(sender=request.user).filter(
                receiver=request.user
            )
        else:
            conversation = all_notes.filter(
                Q(receiver__pk=friend_pk) | Q(sender__pk=friend_pk)
            )
        if conversation:
